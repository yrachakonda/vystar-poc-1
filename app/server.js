...
const storage = multer.diskStorage({
  destination: process.env.FILE_UPLOAD_LOCATION + "/",
...
app.get('/files', (req, res) => {
  fs.readdir(process.env.FILE_UPLOAD_LOCATION, (err, files) => {
    if (err) {
  ...
  upload(req, res, (err) => {
    if (err) {
      ...
      if (req.file == undefined) {
        ...
        console.log('File uploaded:', req.file.filename); // Log the uploaded file name
        res.send({
...
app.get("/download/:filename", (req, res) => {
  const file = `${process.env.FILE_UPLOAD_LOCATION}/${req.params.filename}`;
...
app.post("/users", async (req, res) => {
  ...
  try {
    ...
    console.error("PostgreSQL Error:", error); // Log the error
    res.status(500).send("Internal Server Error");
  }
});
